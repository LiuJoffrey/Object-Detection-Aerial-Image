...
if __name__ == '__main__':
    ...
    val_ann_path = "./improve_det"
    img_file = os.listdir(val_img_path)
    
    for i in range(len(img_file)):
        imgfile = os.path.join(val_img_path, img_file[i])
        detfile = os.path.join(val_ann_path, img_file[i][:-3]+"txt")
        ...
        cv2.imwrite('./impro_img/'+img_file[i][:-4]+'.jpg',image)
